...
class GeneratorTab(ctk.CTkFrame):
    ...
    def __init__(self, parent: ctk.CTk, notification_callback: Callable[[str, str, int], None] = None):
        ...
        # NEW: Material properties variables
        self.add_material_properties_var = ctk.BooleanVar(value=False)
        self.material_properties_entries = {}  # Stores {material_name: {property: entry_widget}}
        self.material_properties_frame = None
        
        # Track whether config files were loaded from project (don't validate existence)
    ...
    def _setup_ui(self):
        ...
        # LEFT SIDEBAR - Project Vehicles (similar to vehicle sidebar)
        ...
        # Sidebar header with project controls
...
# Configuration files section - .pc and .jpg side by side
        ...
        # ===== MATERIAL PROPERTIES SECTION (NEW) =====
        self.material_properties_container = ctk.CTkFrame(skin_card, fg_color="transparent")
        # Will be packed dynamically to ensure correct positioning
        
        # Switch toggle to enable material properties (similar to config data toggle)
        ...
        # Container for material property fields (initially hidden)
        ...
        # Don't pack yet - will be shown when toggle is enabled
        ...
        # Don't pack initially - only shown in editing mode
    ...
    def add_car_to_project(self, carid: str, display_name: str):
        ...
        if carid in self.project_data["cars"]:
            ...
            self.select_car_for_skin(carid)  # Use select_car_for_skin to properly handle editing mode
            return
        ...
        for car_id in self.project_data["cars"].keys():
            if car_id.startswith(f"{carid}_"):
                ...
                self.select_car_for_skin(existing_car_id)  # Use select_car_for_skin to properly handle editing mode
                return
        ...
        # Use select_car_for_skin to properly handle editing mode and form reset
    ...
    def _toggle_car_expansion(self, car_id: str):
        ...
        # If clicking the same car, toggle expansion
        if self.expanded_car_id == car_id:
            self.expanded_car_id = None
        else:
            # Select the car and expand it (this will refresh the display)
    ...
    def select_car_for_skin(self, car_instance_id: str):
        ...
        if car_instance_id in self.project_data["cars"]:
            # If we're currently editing a skin and selecting a different car, cancel editing mode
            ...
            # Reset the form fields when selecting a new car (not editing a skin)
    ...
    def add_skin_to_selected_car(self):
        ...
        # If in editing mode, call update_skin instead
        ...
        if self.add_config_data_var.get():
            ...
            # Additional debugging - check entry widget value
        ...
        # NEW: Handle material properties if enabled
    ...
    def select_skin_for_editing(self, car_instance_id: str, skin_index: int):
        ...
        # Select the car and enter editing mode
        self.selected_car_for_skin = car_instance_id
        ...
        # Populate the form with existing skin data
        # Set skin name - separate try block
        try:
            if self.skin_name_entry:
                self.skin_name_entry.delete(0, "end")
        ...
        # Set DDS path and preview - separate try block
        try:
            if 'dds_path' in skin:
                ...
                try:
                    ...
                    # Clear the label first - this helps prevent "pyimage doesn't exist" errors
                    try:
        ...
        # Set config data if present - SEPARATE try block so DDS errors don't prevent this
        try:
            if 'config_data' in skin:
                ...
                # Update the UI visibility to show config fields FIRST
                self._toggle_config_data()
                print(f"[DEBUG] Called _toggle_config_data to show fields")
                
                # Force update to ensure widgets are visible before setting values
                self.update_idletasks()
                ...
                if 'pc_file_path' in config_data and self.pc_file_entry:
                    # Set the full path in the StringVar (this is what we'll use for validation/saving)
                    self.pc_file_path_var.set(config_data['pc_file_path'])
                    
                    # Mark this path as loaded from project (skip existence validation)
                    self.pc_file_from_project = True
                    
                    # Temporarily unlink the textvariable so we can display just the basename
                    # without affecting the stored full path
                ...
                # No config data - clear all config fields
                self.add_config_data_var.set(False)
                
                # Reset config type to default
                ...
                # Update the UI visibility to hide config fields since no config data
                self._toggle_config_data()
        ...
        # NEW: Set material properties if present
        try:
    ...
    def _update_button_ui(self):
        ...
        if self.editing_mode:
            ...
            # Show Add button and hide Cancel button
    ...
    def update_skin(self):
        ...
        # Update basic skin data
        skin['name'] = skin_name
        skin['dds_path'] = dds_path
        
        # Update config data if enabled
        if self.add_config_data_var.get():
            ...
            # Check if this skin already has config data (loaded from save)
            ...
            # Validate PC file - skip if it's from project and unchanged
            if self.pc_file_from_project and pc_file_path == existing_pc_path:
                ...
                # Path hasn't changed from project load, just verify we have a path
                if not pc_file_path:
                ...
                # Path has changed, so validate it exists
                ...
                # Path hasn't changed, but make sure we have a path
                if not pc_file_path:
                    print(f"[DEBUG] PC file path is empty")
                    self.show_notification("Please select a valid .pc file", "error")
                    return
            
            # Validate JPG file - skip if it's from project and unchanged
            ...
            # Remove config data if checkbox is unchecked
            if 'config_data' in skin:
                del skin['config_data']
        
        # NEW: Update material properties if enabled
        if self.add_material_properties_var.get():
            material_properties = self._collect_material_properties()
            if material_properties:
            ...
            # Remove material properties if checkbox is unchecked
            if 'material_properties' in skin:
    ...
    def _reset_skin_form_fields(self):
        ...
        try:
            # Update UI visibility to match the checkbox state (should be hidden since we set it to False)
            self._toggle_config_data()
        except Exception as e:
            print(f"[DEBUG] Error toggling config data visibility: {e}")
        
        try:
            # CRITICAL FIX: Reset material properties checkbox AND clear the UI
            self.add_material_properties_var.set(False)
            # Properly clear the material properties UI
            if self.material_properties_frame:
                self.material_properties_frame.pack_forget()
                # Clear all widgets to ensure clean state
    ...
    def _toggle_config_data(self):
        ...
        if self.add_config_data_var.get():
            ...
            # Pack before material_properties_container to ensure config always appears above material properties
    ...
    def _browse_pc_file(self):
        ...
        if filename:
            # Store the FULL path in the StringVar - this is what gets validated
            self.pc_file_path_var.set(filename)
            # Clear the "from project" flag since user selected a new file
            self.pc_file_from_project = False
            ...
            # NOTE: The entry widget is linked to pc_file_path_var via textvariable,
            # so setting the StringVar automatically updates the entry.
            # We do NOT manually modify the entry widget to avoid breaking this connection.
    ...
    def _browse_jpg_file(self):
        ...
        if filename:
            ...
            # NOTE: The entry widget is linked to jpg_file_path_var via textvariable,
            # so setting the StringVar automatically updates the entry.
            # We do NOT manually modify the entry widget to avoid breaking this connection.
    ...
    def _toggle_material_properties(self):
        ...
        if self.add_material_properties_var.get():
            try:
                ...
                # CRITICAL FIX: Check if UI already exists and is populated
                # If so, just show it - don't regenerate!
                ...
                # Load material properties from skin.materials.json
                ...
                if not materials:
                    ...
                    # More helpful error message - check if files exist but have no editable properties
                    # Use similar logic to _load_material_structure to find project root
                    project_root = None
                ...
                # Populate the material properties UI
            ...
            # CRITICAL FIX: Only hide, don't destroy widgets when unchecking
            # This preserves user edits if they uncheck and recheck
            if self.material_properties_frame:
                self.material_properties_frame.pack_forget()
    ...
    def _load_material_structure(self, car_id: str) -> Dict:
        ...
        """
        # Find the project root (where vehicles folder is located)
        ...
        # Strategy 2: Script directory (go up from gui/tabs/ to project root)
        if not project_root:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            # script_dir is likely: /path/to/project/gui/tabs/
        ...
        # Strategy 3: Look for vehicles folder in parent directories
        if not project_root:
            current = os.getcwd()
        ...
        material_data = {}
        files_found = []  # Track which material files we found
        
        # Search paths in order of priority
        search_paths = []
        
        # Check if vehicle folder exists in project
        if vehicle_base_path and os.path.exists(vehicle_base_path):
            print(f"[DEBUG] Vehicle folder exists, scanning for subdirectories...")
            # Look for all subdirectories in the vehicle folder (actual skin folders)
            try:
                for item in os.listdir(vehicle_base_path):
                    item_path = os.path.join(vehicle_base_path, item)
                    if os.path.isdir(item_path):
                        # Add any subfolder - this includes SKINNAME, skinname, or any actual skin folder
        ...
        # Also try BeamNG installation as fallback
        try:
        ...
        for search_path in search_paths:
            ...
            # Look for skin.materials.json or materials.json files
            try:
                for filename in os.listdir(search_path):
                    if filename in ['skin.materials.json', 'materials.json']:
                        filepath = os.path.join(search_path, filename)
                        files_found.append(filepath)  # Track that we found a file
                        ...
                        try:
                            ...
                            # BeamNG JSON files often have trailing commas which Python's json doesn't allow
                            ...
                            for material_name, material_info in data.items():
                                # Only include if material has Stages (the actual property data)
                                ...
                                # Collect editable properties from all stages
                                properties = {}
                                for stage_idx, stage in enumerate(stages):
                                    stage_properties = {}
                                    
                                    # Extract the properties we want to edit (include even if null)
                                    for prop in ["clearCoatFactor", "clearCoatRoughnessFactor", "metallicFactor", "roughnessFactor"]:
                                        if prop in stage:  # Include even if value is None
                                ...
                                if properties:
                                    # Extract part name from material name (e.g., "ccf_main" from "ccf_main.skin.skinname")
                            ...
                            if material_data:
                                ...
                                # File found but no editable properties
    ...
    def _populate_material_properties_ui(self, materials: Dict):
        ...
        for material_name, material_info in materials.items():
            ...
            # Material header with part name
            ...
            # Technical name (smaller, grayed out)
            ...
            # Store entries for this material
            self.material_properties_entries[material_name] = {}
            
            # Create entries for each stage's properties
            for stage_key, stage_props in properties.items():
                stage_label = f"Stage {stage_key.split('_')[1]}"
                
                # Stage label
                if len(properties) > 1:  # Only show stage label if multiple stages
                ...
                text_color=state.colors["text"],
                    ...
                    # Display current value (handle null)
            ...
            # Add spacing after each material
    ...
    def _collect_material_properties(self) -> Dict:
        ...
        for material_name, entries in self.material_properties_entries.items():
            ...
            for entry_key, entry_widget in entries.items():
                # Parse entry_key: "stage_0_clearCoatFactor" -> stage=0, prop=clearCoatFactor
                parts = entry_key.split('_', 2)  # Split into: ['stage', '0', 'clearCoatFactor']
                if len(parts) < 3:
                    continue
            ...
            if stages:
                ...
                # Print the final structure for this material
                import json
    ...
    def refresh_project_display(self):
        ...
        # Track which car is expanded (if any)
        ...
        for idx, (car_instance_id, car_info) in enumerate(filtered_cars.items()):
            ...
            # Add skin count to display text
            ...
            # Remove button (overlay on right side)
            ...
            # Skins container (only shown if expanded)
            if is_expanded and car_info["skins"]:
                ...
                for skin_idx, skin in enumerate(car_info["skins"]):
                    ...
                    # Check if this skin is currently being edited
                    ...
                    # Adjust height based on whether skin has config
                    ...
                    # Make skin row clickable to edit
                    ...
                    # Middle: text container (multi-line for config)
                    ...
                    # Skin name (always on first line)
    ...
    def update_current_car_label(self):
        ...
        # Feature disabled - no longer showing "Adding Skins to:" label
        pass
        # if self.selected_car_for_skin and self.selected_car_for_skin in self.project_data["cars"]:
    ...
    def generate_mod(self, generate_button_topbar, output_mode_var, custom_output_var):
        ...
        # Validate config data files exist
        missing_files = []
        for carid, car_info in self.project_data["cars"].items():
